#!/usr/bin/env python3
"""
Привязка вопросов Docker к категории "Docker".
"""
...
try:
    # Находим категорию "Docker" для DevOps (profession_id=4)
    result = db.execute(
        text("SELECT id FROM categories WHERE name = 'Docker' AND profession_id = 4")
    ).first()
    
    if not result:
        print("❌ Категория 'Docker' не найдена!")
        sys.exit(1)
    
    category_id = result[0]
    print(f"✅ Категория 'Docker' найдена (ID={category_id})")
    
    # Получаем ВСЕ вопросы DevOps без категории "Docker"
    # Вопросы Docker имеют ID начиная с 568 (после Linux 448-507 и Сети 508-567)
    result = db.execute(
        text("""
    ...
    print(f"📚 Найдено {total} вопросов без категории Docker")
    
    if not question_ids:
        print("✅ Все вопросы уже привязаны к категории 'Docker'")
        sys.exit(0)
    ...
    print(f"✅ Привязано {bound_count} вопросов к категории 'Docker'")
