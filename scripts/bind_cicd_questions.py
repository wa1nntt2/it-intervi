#!/usr/bin/env python3
"""
Привязка вопросов CI/CD к категории "CI/CD".
"""
...
try:
    # Находим категорию "CI/CD" для DevOps (profession_id=4)
    result = db.execute(
        text("SELECT id FROM categories WHERE name = 'CI/CD' AND profession_id = 4")
    ).first()
    
    if not result:
        print("❌ Категория 'CI/CD' не найдена!")
        sys.exit(1)
    
    category_id = result[0]
    print(f"✅ Категория 'CI/CD' найдена (ID={category_id})")
    
    # Получаем вопросы CI/CD (ID после Docker, примерно с 625)
    # Привязываем все вопросы DevOps без категории CI/CD
    result = db.execute(
        text("""
    ...
    print(f"📚 Найдено {total} вопросов без категории CI/CD")
    
    if not question_ids:
        print("✅ Все вопросы уже привязаны к категории 'CI/CD'")
        sys.exit(0)
    ...
    print(f"✅ Привязано {bound_count} вопросов к категории 'CI/CD'")
