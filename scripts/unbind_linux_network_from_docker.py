#!/usr/bin/env python3
"""
Отвязать вопросы Linux (448-507) и Сети (508-567) от категории Docker.
"""
...
try:
    ...
    if not all([linux_result, network_result, docker_result]):
    ...
    print(f"✅ Linux ID={linux_cat_id}, Сети ID={network_cat_id}, Docker ID={docker_cat_id}")
    
    # Вопросы Linux (ID 448-507) - отвязать от Docker
    ...
    for q_id in linux_question_ids:
        result = db.execute(
            ...
            {"qid": q_id, "cid": docker_cat_id}
        )
        if result.rowcount > 0:
            unbound_linux += 1
    
    # Вопросы Сети (ID 508-567) - отвязать от Docker
    ...
    print(f"✅ Отвязано {unbound_linux} вопросов Linux от Docker")
    print(f"✅ Отвязано {unbound_network} вопросов Сети от Docker")
