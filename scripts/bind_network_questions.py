#!/usr/bin/env python3
"""
Привязка вопросов по сетям к категории "Сети".
"""
...
try:
    # Находим категорию "Сети" для DevOps (profession_id=4)
    result = db.execute(
        text("SELECT id FROM categories WHERE name = 'Сети' AND profession_id = 4")
    ).first()
    
    if not result:
        print("❌ Категория 'Сети' не найдена!")
        sys.exit(1)
    
    category_id = result[0]
    print(f"✅ Категория 'Сети' найдена (ID={category_id})")
    
    # Получаем ВСЕ вопросы DevOps без категории "Сети"
    result = db.execute(
        text("""
    ...
    print(f"📚 Найдено {total} вопросов без категории")
    
    if not question_ids:
        print("✅ Все вопросы уже привязаны к категории 'Сети'")
        sys.exit(0)
    ...
    print(f"✅ Привязано {bound_count} вопросов к категории 'Сети'")
